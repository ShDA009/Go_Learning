...
type Result interface {
    LastInsertId() (int64, error)  // ID последней вставки
    RowsAffected() (int64, error)  // количество затронутых строк
}
```
...
```go
rows.Next() bool               // переход к следующей строке
rows.Scan(dest ...interface{}) // чтение данных
rows.Close() error             // закрытие
rows.Columns() ([]string, error)  // имена столбцов
rows.Err() error               // ошибка итерации
```
...
### Задание 5: Обработка ошибок API
