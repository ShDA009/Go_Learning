...
<Task id="1" points="15">
<Title>Задание 1: Инструментируйте Gin</Title>
<Prompt>
В capstone REST проекте добавьте `otelgin` middleware, проверьте, что на каждый HTTP запрос создаётся span и он виден в Jaeger/Tempo.
</Prompt>
<StarterCode>
```go
package main
...
func main() {
	fmt.Println("otelgin: middleware подключён")
}
```
</StarterCode>
<ExpectedOutput>
otelgin: middleware подключён
</ExpectedOutput>
</Task>

<Task id="2" points="15">
<Title>Задание 2: Инструментируйте gRPC</Title>
<Prompt>
В capstone gRPC проекте добавьте `otelgrpc` interceptors на сервере и клиенте. Проверьте, что у вызова есть server span и client span.
</Prompt>
<StarterCode>
```go
package main
...
func main() {
	fmt.Println("otelgrpc: interceptors подключены")
}
```
</StarterCode>
<ExpectedOutput>
otelgrpc: interceptors подключены
</ExpectedOutput>
</Task>

<Task id="3" points="10">
<Title>Задание 3: Trace ID в логах</Title>
<Prompt>
Добавьте `trace_id`/`span_id` в access log и в логи ошибок. Убедитесь, что по trace_id можно найти все связанные записи.
</Prompt>
<StarterCode>
```go
package main
...
func main() {
	fmt.Println("logs: trace_id/span_id добавлены")
}
```
</StarterCode>
<ExpectedOutput>
logs: trace_id/span_id добавлены
</ExpectedOutput>
</Task>
